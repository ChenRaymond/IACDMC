...
import com.iac.dmc.engine.MultiPointController;
...
public class ControlActivity extends BaseActivity implements OnClickListener {
    ...
    private void initView() {
        ...
        mDevice = DLNARenderContainer.getInstance().getSelectedDevice();

        mPlayMediaIndex = MediaManager.getInstance().getMusicPlayIndex();
        mMediaItemList = MediaManager.getInstance().getMusicList();
        mPlayMediaItem = mMediaItemList.get(mPlayMediaIndex);
        mTotalMusicNum = mMediaItemList.size();
        urls.add(mPlayMediaItem.getRes());
        mPlayUrl = mPlayMediaItem.getRes();
    ...
    private synchronized void play(final String path) {
        ...
        mPlayUrl = mPlayMediaItem.getRes();
        try {
            title = mPlayMediaItem.getTitle();
        }
        catch (Exception e){
            Log.d("Chen Raymond", e.toString());
        }
        mPaused = false;
        showPlay(true);
        ...
        new Thread() {
            @Override
            public void run() {
                final boolean isSuccess = mController.play(mDevice, /*mPlayUrl,*/ mPlayMediaItem);
                Log.w("Chen Raymond", "mPlayUrl " + mPlayUrl);
                if (isSuccess) {
    ...
    private synchronized void playNext() {
        pause();
        mPlayMediaIndex++;
        if (mPlayMediaIndex > mTotalMusicNum - 1) {
            mPlayMediaIndex = 0;
        }
        play(getCurrentPlayPath());
    }
    ...
    private synchronized void playPre() {
        pause();
        mPlayMediaIndex--;
        if (mPlayMediaIndex < 0) {
            mPlayMediaIndex = mTotalMusicNum - 1;
        }
        play(getCurrentPlayPath());
    }
    ...
    private synchronized void getMediaDuration() {
        new Thread() {
            ...
            public void run() {
                ...
                int durationFromMediaItem = mPlayMediaItem.getDuration();
                if(durationFromMediaItem < 1000) {
                    mediaDuration = mController.getMediaDuration(mDevice);
                    mMediaDuration = getIntLength(mediaDuration);
                }
                else {
                    mMediaDuration = durationFromMediaItem/1000;
                    mediaDuration = getTimeString(mMediaDuration);
                }
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if ((TextUtils.isEmpty(mediaDuration) || NOT_IMPLEMENTED.equals(mediaDuration))
                                && mMediaDuration <= 0) {
    ...
    private String getCurrentPlayPath() {
        Log.d("Chen Raymond","mMediaItemList : " + String.valueOf(mPlayMediaIndex));
        MediaManager.getInstance().setMusicPlayIndex(mPlayMediaIndex);
        mPlayMediaItem = mMediaItemList.get(mPlayMediaIndex);
        mPlayUrl = mPlayMediaItem.getRes();
        return mPlayUrl;
    }
    ...
    /**
     * Convert time from seconds to "00:00:00".
     *
     * @param time
     *              time for second
     * @return The string in 00:00:00 format.
     */
